...
function BatchLoaderUnk.create(data_dir, batch_size, seq_length, max_word_l)
  ...
  local in_vocabfile = path.join(data_dir, 'vocab.txt')
  ...
  if not (path.exists(vocab_file) or path.exists(tensor_file) or path.exists(char_file)) then
    ...
    BatchLoaderUnk.text_to_tensor(in_vocabfile, input_files, vocab_file, tensor_file, char_file, max_word_l)
  end
...
function BatchLoaderUnk.text_to_tensor(in_vocabfile, input_files, out_vocabfile, out_tensorfile, out_charfile, max_word_l)
